...
document.getElementById("addItemBtn").addEventListener("click", () => {
  ...
  if (!data || !categoria || !embalagem || isNaN(estoqueAtual)) {
    alert("Preencha todos os campos!");
    return;
  }

  // Exemplo: capacidade fixa (pode vir do cadastro da embalagem)
  ...
  if (editando) {
    ...
    cells[2].textContent = "COD123"; // exemplo
    cells[3].textContent = embalagem;
    cells[4].textContent = capacidade + "ml";
    cells[5].textContent = estoqueAtual;
    cells[6].textContent = pedir;

    editando = null;
    document.getElementById("addItemBtn").textContent = "Adicionar Item";
  } else {
    // Cria nova linha
    adicionarItem(data, categoria, "COD123", embalagem, capacidade + "ml", estoqueAtual, pedir);
  }
